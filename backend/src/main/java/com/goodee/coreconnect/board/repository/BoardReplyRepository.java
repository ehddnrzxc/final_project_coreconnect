...
public interface BoardReplyRepository extends JpaRepository<BoardReply, Integer> {
    ...
    // 특정 게시글의 댓글 목록
    List<BoardReply> findRepliesByBoardId(Integer boardId);

    // 특정 사용자가 작성한 댓글 목록
    List<BoardReply> findRepliesByUserId(Integer userId);

    // 특정 댓글의 대댓글 목록
    List<BoardReply> findRepliesByParentReplyId(Integer parentReplyId);
}
